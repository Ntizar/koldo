...
metadata:
  hermes:
    tags: [forecast, montecarlo, simulacion, escenarios, drivers, prediccion]
    related_skills: [servicio-resumen-consolidado, fetch-paralelo-fallos-parciales]
---
...
- ❌ **Math.random() no determinista** → si se necesitan escenarios reproducibles, usar mulberry32/PCG en vez de Math.random() para Box-Muller
- ❌ **CfD a una sola cara** — en simulaciones de coste del sistema, modelar CfD de doble cara (strike - precioSpot, con signo). Si solo se suma cuando spot < strike, el coste queda sesgado al alza.
