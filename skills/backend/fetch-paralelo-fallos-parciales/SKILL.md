...
metadata:
  hermes:
    tags: [fetch, paralelo, promise-all, fallos, api]
    related_skills: [api-cliente-http-robusto, servicio-resumen-consolidado, cache-multicapa-memoria-disco]
---
...
- ❌ **Cambiar de paralelo a secuencial para "no saturar" sin verificar el impacto real** — la API ESIOS tolera bien batches de 10 en paralelo con concurrency 3. El problema real suele ser timeout individual demasiado bajo (5s) en producción, no el paralelismo. **Paralelismo con batching (3 concurrentes, 10/batch) es más rápido Y más robusto.**
- ❌ **Deploy falla localmente OK** — el servidor local tiene cache (2ms response), pero NaN no. Si el timeout individual es menor que el tiempo real de respuesta de la API, el deploy colgará aunque funcione local. **Siempre limpiar cache y probar sin cache antes de commit.**
