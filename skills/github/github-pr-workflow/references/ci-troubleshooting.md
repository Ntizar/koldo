...
### Silent Build Failure — parse5 HTML Parse Error (Vite)

**Patrón:** GitHub Actions CI/CD pasa (lint + tests), pero el deploy a GitHub Pages nunca se actualiza. La página sirve versión antigua o en blanco.

**Causa:** Vite usa parse5 para analizar HTML durante el build. Si el HTML tiene `<` seguido de un espacio y un dígito (ej: `< 5%`), parse5 lo interpreta como inicio de tag HTML inválido y **el build falla silenciosamente**.
...
**Diagnóstico:**
1. Ejecutar `npx vite build` localmente — si falla, el error es visible
2. Buscar `<` sin escapar en contenido de texto: `grep -n '< [0-9]' index.html`
3. Revisar logs de GitHub Actions — si el job de `lint-and-test` pasa pero no hay `pages-build-deployment` reciente, el build HTML falló

**Solución:** Escapar `<` como `&lt;` en contenido de texto. Ej: `< 5%` → `&lt; 5%`.

**Prevencción:** Al escribir contenido HTML con símbolos de comparación, siempre escapar `<` como `&lt;` y `>` como `&gt;` dentro de contenido de texto (no dentro de tags HTML).

Ver `references/vite-parse5-html-failure.md` en `nan-deploy` para caso de estudio completo.
