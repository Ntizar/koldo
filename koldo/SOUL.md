...
## Skills instalados en Koldo

- **agente-principal.md** — Este archivo, orquestación y comportamiento base
- **voice-setup.md** — Configuración STT/TTS (faster-whisper + Edge TTS, voz Álvaro)
- **secure-api-storage.md** — Gestión segura de API keys (migrar a NaN Cloud Env)
- **dashboard-control-center.md** — NaN Dashboard (Express + Aurora Liquid Glass, puerto 4000)
- **nap-deploy.md** — NAP Dashboard (React + Express, puerto 3000)
...
|| Modelo alternativo | deepseek-v4-flash (284B MoE) |
|| Infraestructura | MicroVM KVM/QEMU, 1 vCPU, 2GB RAM, 20GB disco |
|| Plataforma | NaN.builders (nan.builders) |
|| Gateway | Hermes WebUI (puerto 8787) |
|| Koldo visor web | koldo.apps.nan.builders (nginx) |
|| TTS | Edge TTS, es-ES-AlvaroNeural |
|| STT | faster-whisper (local, tiny) |
|| GitHub | gh CLI autenticado como Ntizar |
|| Repo Koldo | /root/workspace/Koldo (sincronizado) |

## Estado del sistema

- **Repo Koldo**: clonado en `/root/workspace/Koldo`, synced a `/hermes-home/skills/koldo/`
- **Config**: `skills.external_dirs` apunta a `/root/workspace/Koldo/koldo`
- **Cron**: autoconfig diario a 09:00 UTC (job cf21b05773aa)
- **nan-dashboard** (puerto 4000): pendiente de instalar
- **nap-dashboard** (puerto 3000): pendiente de instalar
- **APIs pendientes de migrar a NaN Cloud Env**: GITHUB_TOKEN, ESIOS_API_TOKEN
